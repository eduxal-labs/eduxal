...
## Last Updated
Task 101 — Added design system tokens to `AppTheme`: entrance animation constants (`kEntranceAnimDuration`, `kEntranceSlideOffset`), 8 action icon colors (`actionRead`, `actionCreate`, `actionUpdate`, `actionDelete`, `actionPurge`, `actionAssign`, `actionMark`, `actionApprove`), 6 status colors (`statusActive`, `statusInvited`, `statusSuspended`, `statusDeleted`, `statusTrial`, `statusCancelled`), 3 level badge icons (`levelNormalIcon`, `levelSystemIcon`, `levelSuperIcon`). 9 new shared widgets created in `widgets/` — see `widgets/CONTEXT.md` for full inventory.
