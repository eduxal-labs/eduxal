...
This directory contains **30 files** and **1 subdirectory** (`member_creation/`). These are the shared building blocks that enforce visual consistency across the app — tab bars, term selectors, avatars, save buttons, action buttons, status indicators, sync indicators, progress bars, trajectory indicators, stat badges, member creation flows, today-status cards, quick-stat rows, countdown chips, pressable row mixins, section headers, and the codified design system widgets (sheets, dialogs, form fields, section cards, detail headers, empty states, confirmation dialogs, filter toolbars, search fields).
...
## Last Updated
Tasks H1–H5 — Added 5 new shared widgets: `today_status_card.dart` (TodayStatusCard + TodayStatusType enum), `quick_stat_row.dart` (QuickStat + QuickStatRow), `countdown_chip.dart` (CountdownChip), `pressable_row.dart` (PressableRowMixin), `section_header.dart` (SectionHeader). File count: 30 files + 1 subdirectory (`member_creation/`).
