...
## Last Updated
Tasks F05+F06 — `no_terms_blank_state.dart`: Fixed headline font weight from `w600` to `w500` (UI guideline §21 compliance). Made messaging role-aware: admins see "No terms yet" + CTA; non-admins (students, guardians, teachers/staff without permission) see "No active term" + friendly informational message. File count: 25 files + 1 subdirectory (`member_creation/`).
