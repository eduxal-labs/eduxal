...
## Last Updated
Task E05 — Defense-in-depth permission guards for member status-change actions. `_removeMember()` now checks `Users.Update` before demoting; `_purgeMember()` checks `UserLevel.super_` before purging; `_updateStatus()` checks `Users.Update` (or `Users.Delete` for delete status) before any status change. All show a SnackBar and return early if the check fails. The UI-level `canUpdate`/`canDelete` variables in `build()` already conditionally hide action buttons — the handler-level checks are a second layer of protection. Previous: Tasks D01, E04 — Reactive system permissions via watch stream + lifecycle observer, subjects permission gate with Read guard and CreateSubjectSheet defense-in-depth. Previous: Task E03 — Added confirmation dialogs for destructive system actions. Previous: Task C01 — Fixed logout not navigating away from System Dashboard.