...
## Last Updated
Tasks G7, G8 — `user_detail_sheet.dart`: Added try/catch with SnackBar feedback to `_updateLevel` and `_updateStatus`. `members_section.dart` data source (`watchSystemMembers`) updated in `users_dao.dart` to include Super users (`level >= system`).
