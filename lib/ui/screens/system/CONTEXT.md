...
## Last Updated
Task A1 — `settings/system_settings_screen.dart`: Security fix — `SystemSettingsScreen` now accepts `SystemPermissions` as a required constructor parameter instead of computing permissions inline. The old code granted `SystemPermissions.superUser()` to all System-level users (same as Super), completely bypassing RBAC. Now the caller must provide pre-loaded permissions, ensuring System users receive only their actual scoped permissions. Removed unused `client.dart` and `database/tables/enums.dart` imports.

Previous: Tasks G7, G8 — `user_detail_sheet.dart`: Added try/catch with SnackBar feedback to `_updateLevel` and `_updateStatus`. `members_section.dart` data source (`watchSystemMembers`) updated in `users_dao.dart` to include Super users (`level >= system`).

Previous:
Task B7 — `system_dashboard_screen.dart`: Permission-gated tab visibility. Replaced hardcoded tab index constants (`_kMobileTabHome`, `_kMobileTabMembers`, `_kMobileTabSettings`, `_kMobileTabNotifications`, `_kDesktopTabUsers`, `_kDesktopTabMembers`, `_kDesktopTabSchools`, `_kDesktopTabRoles`) with a dynamic tab system. New `_TabId` enum (7 values) and `_TabItem` class (id + icon + label). `_computeMobileTabs()` and `_computeDesktopTabs()` build filtered lists based on `SystemPermissions`: Users/Members gated by `users.read`, Schools by `schools.read`, Roles by `roles.read`, Settings by `plans.read`; Home and Notifications always visible. `_syncTabControllers()` disposes and recreates `TabController` instances when visible tab count changes (called from all permission-update paths: initial load, watch stream, and app resume). Controllers changed from `late final` to `late`. `_contentForTab(_TabId)` maps tab identity to section widgets. `_showFab` and `_buildFab` use `_TabId` enum matching instead of index constants. `_DesktopBody` gains `List<_TabItem> tabs` parameter — tab bar labels, content children, and `+` button action all derived from the dynamic list. Super users see all tabs unconditionally via `SystemPermissions.superUser()` (all `can()` calls return true).

Previous: Task B8 — `home/system_stats_section.dart`: Added per-card permission gating to `_StatsCardGrid.build()`. Each of the 6 stat cards is now conditionally included using `permissions.can(Resource.xxx, Action.read)`: Users → `users.read`, Schools → `schools.read`, Students → `students.read`, Teachers → `teachers.read`, Subscriptions + Revenue → `plans.read`. Super users bypass all checks and see all cards. Added `import '../../../../models/permissions.dart' show Action, Resource;` and changed `flutter/material.dart` import to `hide Action` to resolve the name collision (same pattern as `user_detail_sheet.dart`). Call site already passes `permissions` — no changes needed.
