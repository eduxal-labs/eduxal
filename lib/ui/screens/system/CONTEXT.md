...
| `user_detail_sheet.dart` | `UserDetailSheet` | ✅ Complete | Slide-over/dialog showing user details — name, phone, email, level, status, created/updated timestamps. Edit actions for level and status changes. **Security:** "Elevate to super" action is gated to Super-only viewers (prevents System privilege escalation). Self-action guard: `_AccountActionsCard` returns `SizedBox.shrink()` when `user.id == viewerId`, preventing users from demoting/suspending/deleting themselves. `viewerId` is threaded from `cache.currentUser` through `_ViewBody` → `_AccountActionsCard`. |
