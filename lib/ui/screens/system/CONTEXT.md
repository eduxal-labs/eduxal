...
## Last Updated
Task A2 — Fixed `InviteUserSheet` keyboard handling after `EduSheet` wrapper removal (Task A0). Changed `SingleChildScrollView` bottom padding from `0` to `16` so form fields scroll with a small gap above the submit button. Verified `_SubmitButton` area uses `MediaQuery.viewInsetsOf(context).bottom + 24` as the sole viewInsets handler. Previous: Task B3.