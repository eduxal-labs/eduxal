...
| `subjects_section.dart` | `SubjectsSection` | ✅ Complete | Global subject & topic catalog management. Each topic's expanded content (`_TopicExpandedContent`) now includes a **Questions panel** with: FutureBuilder-driven question count badge (via `questionBankService.listQuestions`), permission-gated "Add question" button (`Subjects.create`) that opens `CreateQuestionSheet` via `showEduSheet`, "Import questions" button, "View all questions →" navigation row (navigates to `QuestionsListPage`), and "Bulk import" row (placeholder pending Task 10). `_TopicExpandedContent` converted from StatelessWidget to StatefulWidget to cache the count future; now accepts `canEdit` and `canDelete` props (passed through from `_TopicTile`) for forwarding to `QuestionsListPage`. `_TopicTile` and `_TopicList` pass `canCreate`, `canEdit`, `canDelete` down the widget tree. |
...
**Dependencies:** `client.dart` (`questionBankService`, `accessToken`), `models/result.dart` (`Ok`/`Err`), `models/question.dart` (`RubricCriterion`, `QuestionImage`, `ImageContext`), `models/permissions.dart`, `models/system_permissions.dart`, `database/daos/catalog_dao.dart`, `ui/theme/app_theme.dart`, `ui/widgets/edu_sheet.dart`, `ui/widgets/edu_confirm_dialog.dart`, `ui/widgets/edu_form_field.dart`.
...
## Last Updated
Task 09 — Added `questions_list_page.dart` (`QuestionsListPage`) for full-page question browsing with pagination, inline expansion, edit (`_EditQuestionSheet`), and delete. Wired "View all questions" navigation in `subjects_section.dart` `_TopicExpandedContent` (replaced Task 09 TODO). Added `canEdit`/`canDelete` props to `_TopicExpandedContent` and threaded them from `_TopicTile`. Previous: Task 08.