...
## Last Updated
- Task X01 — **Fixed window resize ejecting user from current page.** Both `school_dashboard_screen.dart` and `system_dashboard_screen.dart` previously created separate `Scaffold` widgets per layout tier (full sidebar / icon rail / mobile tabs for school; mobile / desktop for system). Crossing a breakpoint tore down the entire widget tree, destroying any `Navigator.push`-based detail pages (exam detail, paper detail, role detail, school detail, etc.). **School dashboard fix:** Added `_LayoutMode` enum (`full`, `rail`, `mobile`) and refactored `_DashboardShellState.build()` to use a single `Scaffold` with a `LayoutBuilder` that schedules `_layoutMode` updates via `addPostFrameCallback`. The `_buildLayout()` method uses a `switch` expression on `_LayoutMode` to swap only the navigation chrome (sidebar / rail / tabs) while keeping the content area widget tree persistent. Mobile layout no longer uses `TabBarView` — content is driven by `_selectedIndex` like the other layouts. **System dashboard fix:** Replaced `MediaQuery.sizeOf(context).width` conditional with a `LayoutBuilder` + `_isMobile` state flag + single `Scaffold`. `_buildMobileBody()` and `_buildDesktopBody()` return body content (not separate `Scaffold`s), composed into one `Scaffold` in `_buildLayout()`. Both files compile cleanly: 0 errors, 0 warnings.
