  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<StudentFinanceSummary>(
      ...
      builder: (context, snap) {
        ...
        final summary = snap.data!;
        final unpaidInvoices = summary.invoices
            .where((inv) => !inv.isFullyPaid)
            .toList();

        return Stack(
          children: [
            _GuardianFinanceContent(
              summary: summary,
              studentName: widget.studentName,
              cs: cs,
              isDark: isDark,
              bottomPadding: unpaidInvoices.isNotEmpty ? 72.0 : 0.0,
            ),
            if (unpaidInvoices.isNotEmpty)
              Positioned(
                ...
                child: FloatingActionButton.extended(
                  onPressed: () => _showGuardianPaymentSheet(
                    context,
                    unpaidInvoices,
                    _dao,
                    _schoolId,
...
// ═════════════════════════════════════════════════════════════════════════════
// BOTTOM SHEETS — Guardian Payment / Create Fee / Record Payment
// ═════════════════════════════════════════════════════════════════════════════

Future<void> _showGuardianPaymentSheet(
  BuildContext context,
  List<InvoiceWithDetails> unpaidInvoices,
  FinanceDao dao,
  String schoolId,
  int studentAdm,
) {
  return showEduSheet(
    context: context,
    title: 'Make Payment',
    builder: (_) => _GuardianPaymentSheet(
...
class _GuardianPaymentSheet extends StatefulWidget {
  const _GuardianPaymentSheet({
  ...
  final List<InvoiceWithDetails> unpaidInvoices;
  final FinanceDao dao;
  final String schoolId;
  final int studentAdm;

  @override
  State<_GuardianPaymentSheet> createState() => _GuardianPaymentSheetState();
}

class _GuardianPaymentSheetState extends State<_GuardianPaymentSheet> {
  final _formKey = GlobalKey<FormState>();
  final _amountCtrl = TextEditingController();
  final _refCtrl = TextEditingController();
  PaymentMethod _method = PaymentMethod.cash;
  bool _saving = false;
  late InvoiceWithDetails _selectedInvoice;

  @override
  void initState() {
    super.initState();
    _selectedInvoice = widget.unpaidInvoices.first;
    _amountCtrl.text = _selectedInvoice.balance.toStringAsFixed(2);
  }
  ...
  void _onInvoiceChanged(InvoiceWithDetails? inv) {
    if (inv == null) return;
    setState(() {
  ...
  Future<void> _save() async {
    ...
    try {
      final id =
          '${_selectedInvoice.invoice.id}_pay_${DateTime.now().millisecondsSinceEpoch}';
      final amount = double.parse(_amountCtrl.text.trim());
      final ref = _refCtrl.text.trim();

      await widget.dao.recordPayment(
        id: id,
        invoiceId: _selectedInvoice.invoice.id,
        schoolId: widget.schoolId,
        studentAdm: widget.studentAdm,
        amount: amount,
        method: _method,
  ...
  Widget build(BuildContext context) {
    ...
    final invoiceTitle =
        _selectedInvoice.feeTitle ??
        _selectedInvoice.invoice.description ??
        'Invoice';
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    // ── Invoice picker ──────────────────────────────────
                    if (widget.unpaidInvoices.length > 1) ...[
                      Text(
                        'Select Invoice',
                        style: TextStyle(
                          fontSize: 12,
                      ...
                      const SizedBox(height: 6),
                      DropdownButtonFormField<InvoiceWithDetails>(
                        ...
                        items: widget.unpaidInvoices.map((inv) {
                          final title =
                              inv.feeTitle ??
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.w500,
                            color: _selectedInvoice.balance > 0.01
                                ? _kOverdueColor
                                : _kPaidColor,
                          ),
                        ),
