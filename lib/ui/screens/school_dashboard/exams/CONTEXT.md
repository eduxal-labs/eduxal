...
## Last Updated
Task 01 (fix) — UX improvements to `paper_generation_page.dart`: inline error banner replaces SnackBar on generation failure (`_generateError` state + `_friendlyGenerateError` helper + `_GenerationErrorBanner` widget), loading overlay shown over topic list during generation (`Stack` + `Positioned.fill`), and `addPostFrameCallback` accumulation fixed via `_lastSyncedTopics` guard (callback only fires when topic list actually changes).
...
| `paper_pdf_viewer.dart` | ✅ Active | `downloadAndOpenPdf` — top-level async function that fetches a presigned PDF URL via `QuestionBankService.getPaperPdf`, downloads bytes via `HttpClient`, saves to temp dir, and opens with the system viewer (`xdg-open`/`open`/`start`). Used by both `paper_generation_page.dart` (Step 2 finalize) and `paper_detail_page.dart` (Print Paper button on done/marked papers). |
...
## Last Updated
Task 01 (fix) — Inline error banner, loading overlay, and callback-leak guard added to `paper_generation_page.dart`. See file entry above for full details.