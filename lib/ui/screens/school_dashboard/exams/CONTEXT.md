...
## Last Updated
Task 04 — `lib/services/question_bank.dart`: `getPaperQuestions` method added to `QuestionBankService`. `lib/ui/screens/school_dashboard/academics/paper_generation_page.dart`: `_tryRestoreExistingQuestions` added to `_PaperGenerationPageState`; called at end of `initState` to restore wizard to review step (step 1) when the server already has questions for this paper.
...
## Last Updated
Task 04 — `getPaperQuestions` method added to `QuestionBankService` in `lib/services/question_bank.dart`; `_tryRestoreExistingQuestions` added to `_PaperGenerationPageState` in `paper_generation_page.dart` to restore wizard state on re-entry.