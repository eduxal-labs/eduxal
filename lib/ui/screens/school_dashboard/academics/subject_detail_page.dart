...
class _CreatePaperSheetState extends State<_CreatePaperSheet>
  ...
  Future<void> _syncAutoName(StateSetter setModalState) async {
    ...
    final topics = await catalogDao.getTopicsByIds(_selectedTopicIds.toList());
    if (topics.isEmpty) return;

    if (isAssessment) {
      final topic = topics.first;
      final seq = await widget.dao.getNextTopicPaperNumber(
        schoolId: widget.schoolId,
      ...
      final generated = 'Assessment $seq of ${topic.name}';
      _autoGeneratedName = generated;
      setModalState(() {
        _nameCtrl.text = generated;
      });
    } else {
      final topicNames = topics.map((t) => t.name).toList();
      final topicTitle = topicNames.join(', ');
      final seq = await widget.dao.getNextTopicPaperNumber(
        schoolId: widget.schoolId,
        subject: widget.subjectId,
        grade: _selectedGrade,
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText: isAssessment
                            ? 'e.g. Assessment 1 of Topic'
                            : 'e.g. Assignment 1 of Topic',
                        filled: true,
                        fillColor: isDark
