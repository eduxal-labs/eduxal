...
Domain models for the AI question bank. Imports: `dart:convert`, `proto/services/question_bank.pb.dart`.

- **`ImageContext`** — Enum: `question`, `rubric`, `exampleAnswer`.
- **`RubricCriterion`** — One rubric criterion with mark allocation. Fields: `criterion` (String), `marks` (int). Factories: `fromProto(pb.RubricCriterion)`, `fromMap(Map<String,dynamic>)`.
- **`QuestionImage`** — Image attached to a question (client-side only — `pb.QuestionImage` no longer exists in the proto). Fields: `context` (ImageContext), `filename` (String), `caption` (String?), `description` (String), `getUrl` (String?). **No `fromProto` factory** (removed when proto dropped `QuestionImage`).
- **`QuestionPart`** — A labelled sub-question part within a structured question. Fields: `label` (String — e.g. 'a','b','i'), `body` (String), `bodyFormat` ('plain'|'tiptap'), `marks` (int), `answerSpaceType` ('lines'|'plain_box'|'diagram_box'|'construction_box'|'grid_box'), `answerLines` (int), `answerBoxHeightMm` (int), `rubric` (List\<RubricCriterion\>), `exampleAnswer` (dynamic). Factory: `fromMap(Map<String,dynamic>)`.
- **`Question`** — A question in the question bank. Fields: `id` (int), `topicId` (int), `text` (String — legacy alias for `body`), `body` (String), `bodyFormat` ('plain'|'tiptap'), `parts` (List\<QuestionPart\>), `stimulus` (Map\<String,dynamic\>? — JSON-decoded from proto string), `type` (String — 'definition'|'explanation'|'calculation'|'structured'|'experiment'|'data_response'|'diagram'), `difficulty` (int 1–5), `cognitiveLevel` (String — 'recall'|'comprehension'|'application'|'analysis'), `marks` (int), `maxMarks` (int), `answerSpaceType` (String), `answerLines` (int), `rubric` (List\<RubricCriterion\>), `exampleAnswer` (dynamic), `images` (List\<QuestionImage\> — always `const []` when populated from proto), `created` (DateTime), `updated` (DateTime). Constructor: `body` defaults to `text`, `maxMarks` defaults to `marks`. Static helpers: `_bodyFmtStr(int)`, `_typeStrs`, `_cognitiveStrs`, `_answerSpaceStrs`. Factory: `fromProto(pb.Question)` — proto has `body` (not `text`), no `images` field; `created`/`updated` are Int64 seconds since epoch.
- **`BulkImportResult`** — Result of bulk import. Fields: `createdCount` (int), `questionIds` (List\<int\> — always empty, proto no longer returns IDs), `errors` (List\<ImportError\> — always empty, proto no longer returns per-question errors). Factory: `fromProto(pb.BulkImportResponse)` — maps `proto.created` → `createdCount`.
- **`ImportError`** — Single error from bulk import. Fields: `index` (int), `message` (String). **No `fromProto` factory** (removed when proto dropped `ImportError`). Used for local error display only.

### Paper Generation models — `paper_generation.dart`
Domain models for AI paper generation flow. Imports: `dart:convert`, `question.dart` (for `RubricCriterion`, `QuestionImage`, `QuestionPart`), `proto/services/question_bank.pb.dart`, `package:fixnum/fixnum.dart` (for `Int64.ZERO` comparisons).
...
- **`PaperQuestion`** — Generated question for a paper before finalization. Fields: `id` (String), `questionId` (int), `text` (String — legacy alias for `body`), `body` (String), `bodyFormat` ('plain'|'tiptap'), `parts` (List\<QuestionPart\>), `stimulus` (Map\<String,dynamic\>?), `type` (String), `difficulty` (int 1–5), `marks` (int), `rubric` (List\<RubricCriterion\>), `images` (List\<QuestionImage\> — always `const []`), `answerSpaceType` (String), `answerLines` (int), `order` (int — always 0 when from proto, position no longer in wrapper), `section` (String? — always null when from proto). Factory: `fromProto(pb.Question)` — **signature changed from `pb.PaperQuestion` to `pb.Question`** (proto dropped the `PaperQuestion` wrapper; `GetPaperQuestionsResponse.questions` now returns `List<Question>` directly). Inlines same int-to-string enum helpers as `Question`.
- **`PaperPdf`** — Result of paper finalization with PDF URL. Fields: `pdfUrl` (String), `pdfExpiry` (DateTime). Factories: `fromProto(pb.FinalizePaperResponse)`, `fromGetPdfProto(pb.GetPaperPdfResponse)` — both convert Int64 seconds to DateTime. ⚠️ `FinalizePaperResponse`/`GetPaperPdfResponse` fields changed in M0 — `fromProto` factories have compile errors pending a separate fix.
