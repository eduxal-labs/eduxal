...
Task E1+E2 — Verified `clearAndSeed` deletes `topics`, `streams`, and `mpesa` tables in correct FK order (topics before subjects, streams/mpesa before schools). Verified `_seedAll()` purges the `logs` table after seeding so demo data does not flood the sync engine.