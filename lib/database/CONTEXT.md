...
## Last Updated
Task AUTH-B06 — Authorization checks wired into all remaining DAO mutation methods:
- `lib/database/daos/departments_dao.dart` — added `import '../../services/authorization_service.dart'`. Added pre-flight `authorization.check(...)` + `PermissionException` throw in all 5 mutation methods: `createDepartment` (`SyncAction.createDepartment`, `schoolId: companion.school.value`), `updateDepartmentDescription` (`SyncAction.updateDepartment`, `schoolId: schoolId`), `assignTeacherToDepartment` (`SyncAction.updateTeacher`, `schoolId: schoolId`), `assignStaffToDepartment` (`SyncAction.updateStaff`, `schoolId: schoolId`), `deleteDepartment` (`SyncAction.deleteDepartment`, `schoolId: schoolId`).
- `lib/database/daos/terms_dao.dart` — added import. Auth checks in: `createTerm` (`SyncAction.createTerm`, `schoolId: term.school.value`), `updateTerm` (`SyncAction.updateTerm`, `schoolId: schoolId`), `deleteTerm` (`SyncAction.deleteTerm`, `schoolId: schoolId`). The `upsertTerm` sync-sourced write is not guarded (server-originated).
- `lib/database/daos/schools_dao.dart` — added import. Auth checks in: `createSchool` (`SyncAction.createSchool`, `schoolId: null` — system-level), `updateSchoolDetails` (`SyncAction.updateSchool`, `schoolId: schoolId`), `purgeSchool` (`SyncAction.deleteSchool`, `schoolId: schoolId`), `linkOwner` (`SyncAction.createOwner`, `schoolId: schoolId`). `updateSchoolStatus` delegates to `updateSchoolDetails` — guarded indirectly.
- `lib/database/daos/members_dao.dart` — added import. Auth checks on all creation-path methods: `inviteAndAddOwner` + `addExistingUserAsOwner` (`SyncAction.createOwner`, `schoolId: owner.school.value`), `inviteAndAddTeacher` + `addExistingUserAsTeacher` (`SyncAction.createTeacher`, `schoolId: teacher.school.value`), `inviteAndAddStaff` + `addExistingUserAsStaff` (`SyncAction.createStaff`, `schoolId: member.school.value`), `createStudent` (`SyncAction.createStudent`, `schoolId: student.school.value`), `inviteAndAddGuardian` + `addExistingUserAsGuardian` (`SyncAction.createGuardian`, `schoolId: guardian.school.value`). Update/remove methods are guarded at the service layer via `member_management.dart`.
- `lib/database/daos/enrollments_dao.dart` — added import. Auth checks in: `enrollStudent` (`SyncAction.enrollStudent`, `schoolId: schoolId`), `unenrollStudent` (`SyncAction.unenrollStudent`, `schoolId: schoolId`). `bulkEnroll` delegates to `enrollStudent` — guarded indirectly.
- `lib/database/daos/plans_dao.dart` — added import. Auth checks in: `createPlan` (`SyncAction.createPlan`, `schoolId: null` — system), `updatePlan` (`SyncAction.updatePlan`, `schoolId: null`), `purgePlan` (`SyncAction.deletePlan`, `schoolId: null`), `createSubscription` (`SyncAction.createSubscription`, `schoolId: sub.school.value`), `updateSubscriptionStatus` (`SyncAction.updateSubscription`, `schoolId: schoolId`). Discounts remain in `finance_dao.dart` (already guarded in AUTH-B02).
- `lib/database/daos/school_scopes_dao.dart` — added import. Auth checks in: `createRole` (`SyncAction.createRole`, `schoolId: companion.school.present ? companion.school.value : null`), `updateRole` (`SyncAction.updateRole`, `schoolId: null` — system-level per `_resolveOrganisation`), `deleteRole` (`SyncAction.deleteRole`, `schoolId: null`), `assignRole` (`SyncAction.assignRole`, `schoolId: schoolId`), `unassignRole` (`SyncAction.unassignRole`, `schoolId: schoolId`).
- `lib/database/daos/catalog_dao.dart` — added import. Auth checks in all 12 mutation methods: `createSubject`/`updateSubject`/`deleteSubject` (`schoolId: null` — system), `createTopic`/`updateTopic`/`deleteTopic` (`schoolId: null`), `createSchoolStream`/`updateStream`/`deleteStream`/`deleteAllStreamsForGrade` (`schoolId: schoolId`), `upsertMpesa` (pre-check reads `getMpesa(schoolId)` before transaction to decide `createMpesa` vs `updateMpesa`), `deleteMpesa` (`SyncAction.deleteMpesa`, `schoolId: schoolId`).
- `lib/services/member_management.dart` — added `authorization.check(...)` call after the `accountId` null check in all 12 public mutation methods: `updateTeacher` + `changeTeacherStatus` → `SyncAction.updateTeacher`; `removeTeacher` → `SyncAction.deleteTeacher`; `updateStaff` + `changeStaffStatus` → `SyncAction.updateStaff`; `removeStaff` → `SyncAction.deleteStaff`; `removeOwner` → `SyncAction.deleteOwner`; `updateStudent` + `changeStudentStatus` → `SyncAction.updateStudent`; `removeStudent` → `SyncAction.deleteStudent`; `updateGuardian` → `SyncAction.updateGuardian`; `removeGuardian` → `SyncAction.deleteGuardian`. Uses `return Err(MemberActionError.permissionDenied)` on denial (no throw). Existing `SchoolPermissions`-based defense-in-depth guards retained below the new check. No new import needed (`authorization` is accessible via `client.dart`).

Previous: Task AUTH-B05 — Authorization checks added to all `TimetableDao` mutation methods:
