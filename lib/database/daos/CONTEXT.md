...
## Last Updated
Task TT-02 — Added `watchSchoolWideTimetable({required String schoolId, required int year, required int term}) → Stream<List<SchoolWideTimetableEntry>>` to `TimetableDao`. Added `SchoolWideTimetableEntry` data class (fields: `day`, `startTime`, `endTime`, `grade`, `stream`, `subjectId`, `subjectName`, `teacherId`, `teacherName`). Method uses `innerJoin(users)` + `leftOuterJoin(subjects)`, ordered by day then start time. Previous: Task A3 — Added `getSubjectTeachersForTerm` and `SolverAssignment`.