...
## Last Updated
Task A3 — Updated `RolesDao` sync payload encoding: `createRole()` and `updateRole()` now convert JSON text to binary blob via `parsePermissions()` + `Permissions.toBlob()` for protobuf payloads. Previous: Task D2 — Added `isClassTeacherFor` method to `MembersDao` for teacher attendance marking restriction.