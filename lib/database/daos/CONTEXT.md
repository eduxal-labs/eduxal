...
## DAOs Updated in Task D2 (authorization check on createExamWithPapers)

- **`exams_grades_dao.dart`** — `createExamWithPapers()` now calls `authorization.check(SyncAction.createExam)` before the transaction block, matching the pattern used by all other mutation methods. Previously lacked any authorization check, making the `PermissionException` catch in `_CreateExamFromGradeSheet._save()` dead code.
