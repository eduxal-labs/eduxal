...
- `watchExamGroups({required String schoolId, required int year, required int term}) → Stream<List<ExamGroup>>` — Emits all exams for a term grouped by `(type, start, end)` into `ExamGroup` objects. Each group contains `ExamGradeEntry` per grade, each with `ExamStreamEntry` per stream (with papers). Sorted by start date descending (most recent first). Uses `asyncMap` to join teacher user data and papers.
...
## Last Updated
- Schema v2 Migration — Updated `academics_dao.dart` and `exams_grades_dao.dart` to use renamed `SubjectTeachers` table (was `Subjects`) and removed all `mastery.grade` column references (mastery PK is now `(school, student, subject, topic)`). Also updated exam mutation methods (`createExam`, `createExamBatch`, `updateExam`, `addGradeToExamGroup`) to reflect that the `exams` table no longer has `grade` or `stream` columns, replacing `grade` with `name` in `CreateExamPayload`.
- Task 24 — Added `PaperSubmissions` table to `ExamsGradesDao`'s `@DriftAccessor` tables list. Added 5 new paper submission methods: `getSubmissionsForStudent({schoolId, examId, student, subject, paperNum}) → Future<List<PaperSubmissionData>>` (all submission rows for one student+paper, ordered by createdAt asc); `getSubmissionsForPaper({schoolId, examId, subject, paperNum}) → Future<Map<int, List<String>>>` (all submissions for an entire paper keyed by student adm); `insertSubmission({schoolId, examId, student, subject, paperNum, path}) → Future<void>` (insert or replace on conflict); `deleteSubmission({schoolId, examId, student, subject, paperNum, path}) → Future<void>` (delete a single path row); `clearSubmissionsForStudent({schoolId, examId, student, subject, paperNum}) → Future<void>` (delete all paths for a student+paper). None of these methods write to the `logs` table — `paper_submissions` is client-only and never synced.
- Task 21 — Added `watchPaper` reactive stream method to `ExamsGradesDao`. New method signature: `watchPaper({required String schoolId, required String examId, required int subject, required int? paperNum}) → Stream<Paper?>`. Watches a single paper row by composite key via `watchSingleOrNull()`, emitting null if deleted. Added near `getPaper` (line ~376). Used by `paper_detail_page.dart` to fix stale status bug where status chips and action buttons did not update until page re-entry.
- Task EX1 — Added `watchExamGroups`, `updateExamGroupDateRange`, `addGradeToExamGroup`, `_generateExamId` methods to `ExamsGradesDao`. Added `import 'dart:math' as math'` and `import '../../models/exam_group.dart'`. These methods support the new grouped exam UI: grouping exam rows by `(type, start, end)`, updating date ranges atomically across all rows in a group, and adding new grades to an existing exam group.